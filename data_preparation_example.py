...
re_size=224
imgs_all = np.zeros((num, 3, re_size, re_size), dtype=np.uint8)
...
for i in np.arange(0, num):
    ...
    # plt.imshow(im)
    # plt.show()
    imgs_all[i] = cv2.resize(im, (re_size, re_size), interpolation=cv2.INTER_CUBIC).transpose(2, 0, 1)
...
np.savez('Koniq_768.npz', X=imgs_all[ind_train],Y=mos[ind_train], Xtest=imgs_all[ind_test], Ytest=mos[ind_test])
