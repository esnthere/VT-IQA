...
def main():
    ...
    model.load_state_dict(torch.load('VTIQA_cid2013.pt'))
...
# # #####################################
    ...
    del all_data
    X = np.concatenate((X, Xtest), 0)
    Y = np.concatenate((Y, Ytest), 0)
    X2 = np.concatenate((X2, Xtest2), 0)
    ind = np.arange(X.shape[0])
    np.random.seed(seed=4)
    np.random.shuffle(ind)
    Xtest = X[ind[int(X.shape[0] * 0.8):]]
    Ytest = Y[ind[int(Y.shape[0] * 0.8):]]
    Xtest2 = X2[ind[int(X2.shape[0] * 0.8):]]
